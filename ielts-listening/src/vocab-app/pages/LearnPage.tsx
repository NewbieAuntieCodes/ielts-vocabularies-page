...
const LearnStep: React.FC<{ 
    ...
    const safeIndex = Math.min(currentIndex, topicWords.length - 1);
    const currentWord = topicWords[safeIndex];
    ...
    const handleNext = () => setCurrentIndex(i => Math.min(i + 1, topicWords.length - 1));
    ...
    return (
        <StepContainer>
            ...
            <FlashcardNav>
                ...
                <ProgressText>{safeIndex + 1} / {topicWords.length}</ProgressText>
                <NavButton onClick={handleNext} disabled={safeIndex === topicWords.length - 1}><NextIcon/></NavButton>
            </FlashcardNav>
